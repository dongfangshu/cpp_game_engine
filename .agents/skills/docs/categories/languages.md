...
C and C++ are covered by the core skills under [`basics/`](./basics.md), [`project-config/`](./project-config.md), [`toolchains/`](./toolchains.md), and [`packages/`](./packages.md) — there is no dedicated `xmake-cpp` skill because it's the default everywhere.
