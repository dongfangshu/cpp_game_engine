...
- How big / how aligned is a **type**? (`check_sizeof`, `check_alignof`)
...
import("lib.detect.check_alignof")
...
target("foo")
    on_config(function (target)
        print("sizeof(void*)  = %d", target:check_sizeof("void*"))
        print("alignof(double) = %d", target:check_alignof("double"))
        -- with a header and extra configs
        local n = target:check_alignof("struct foo", {includes = "foo.h"})
    end)
```

Both return the value or `nil` when the type does not compile, and both respect the
target's toolchain and flags, so the numbers are the ones of the *target* platform, not of
the host. `check_alignof` needs xmake >= v3.1.0.
